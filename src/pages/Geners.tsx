...
const Geners = () => {
    ...
    const { data, isLoading,fetchNextPage,isFetchingNextPage,hasNextPage,isError,error,refetch} = useFetchInfine<MediaResponse>({
        endpoint:`discover/${media_Type}?with_genres=${id}&language=en-US`,
        keys:[genre!,media_Type!,id!]
 ...
 if (isLoading) {
  ...
  if (!data || data.pages.length === 0) {
    return (
      <StateMessage
        title="No genre results found"
        message="There are no titles in this genre right now."
        className="min-h-[70vh]"
      />
    );
  }
  ...
  if (results.length === 0) {
    return (
      <StateMessage
        title="No genre results found"
        message="There are no English titles in this genre right now."
        className="min-h-[70vh]"
      />
    );
  }
